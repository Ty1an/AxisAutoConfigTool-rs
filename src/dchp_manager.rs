...
impl DhcpManager {
    ...
    async fn generate_ip_pool(&self) -> Result<(), DhcpError> {
        ...
        *self.available_ips.write().await = ips;
        ...
        info!("Generated IP pool with {} available addresses", 
    ...
    async fn handle_dhcp_discover(&self, packet: &DhcpPacket, socket: &UdpSocket) -> Result<(), DhcpError> {
    ...
    async fn handle_dhcp_request(&self, packet: &DhcpPacket, socket: &UdpSocket) -> Result<(), DhcpError> {
        let mac = packet.get_mac();
        let now = Utc::now();
        ...
        if let Some(lease) = leases.get_mut(&mac) {
            ...
            info!("DHCP lease renewed for MAC {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}, IP {}",
    ...
    /// Send DHCP OFFER
    async fn send_dhcp_offer(&self, packet: &DhcpPacket, offer_ip: Ipv4Addr, socket: &UdpSocket) -> Result<(), DhcpError> {
    ...
    /// Send DHCP ACK
    async fn send_dhcp_ack(&self, packet: &DhcpPacket, ack_ip: Ipv4Addr, socket: &UdpSocket) -> Result<(), DhcpError> {
        let server_ip = self.server_ip.unwrap();
        let lease_time = self.lease_time.as_secs() as u32;
        ...
        let mac = packet.get_mac();
        info!("Sent DHCP ACK: {} to {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
